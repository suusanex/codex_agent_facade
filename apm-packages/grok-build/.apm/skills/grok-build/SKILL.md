---
name: grok-build
description: Codex 上のユーザー本文を Codex Agent Facade の MCP tool run_agent 経由で Grok Build CLI へ渡す。この Codex thread で Grok Build に実装させたいときに使う。
user-invocable: true
...
- `session_id`: 同じ Grok session を続けるときは、この thread の直前の `run_agent` 結果の `sessionId`
