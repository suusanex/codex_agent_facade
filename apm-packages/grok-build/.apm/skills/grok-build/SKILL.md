---
name: grok-build
description: Codex 自身は対象作業を実行しない。Skill より後のユーザー本文を Codex Agent Facade 経由で Grok Build CLI へ委譲し、その結果を中継する。この Codex thread で Grok Build に作業させ、Codex は薄い UI shell / relay として結果だけを返すときに使う。
user-invocable: true
...
# Grok Build（Codex Agent Facade）

この Skill が指定された turn で、Codex は planner / executor / reviewer / orchestrator ではない。Codex は Grok Build に対する薄い UI shell / relay である。対象作業は Grok Build が行い、Codex はその結果を中継する。
...
`session_id` の lifetime は異なる。同じ Grok session をユーザー turn をまたいで続けるときは、直前の completed `result.sessionId` を再利用する。`session_id` の継続は `request_id` の再利用理由にならない。
...
2. この Skill の規約に従って `start_agent` の引数を、呼び出しごとに完全な RPC として機械的に構成する。
   ...
   - `session_id`: 同じ Grok session を続けるときは、この thread の直前の completed `result.sessionId`
...
8. 次の turn で同一 Grok session を継続できるよう `result.sessionId` を保持する。この値は次の Follow-up continuation の `session_id` であり、次の `request_id` ではない。
...
- 前回 job が completed
- 新しいユーザー依頼 / 新しい payload を同じ Grok session に続けて渡す
...
同じ Codex thread でも、同じ Grok session でも、新しい `request_id` を使う。前回 completed job の `request_id` は使わない。
