...
$scratchPath = Join-Path $tempRoot ('grok-build-apm-' + [guid]::NewGuid().ToString('N'))
$safeToDelete = $false
...
try {
    ...
    $sourceSkill = Join-Path $packageRoot '.apm/skills/grok-build/SKILL.md'
    $installedSkill = Join-Path $resolvedScratch '.agents/skills/grok-build/SKILL.md'
    if (-not (Test-Path -LiteralPath $installedSkill -PathType Leaf)) {
        throw 'APM installation did not deploy grok-build/SKILL.md.'
    }
    ...
    Write-Output 'Grok Build package-root APM install smoke test: PASS'
}
finally {
