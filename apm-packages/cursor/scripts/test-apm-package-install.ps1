...
$scratchPath = Join-Path $tempRoot ('cursor-apm-' + [guid]::NewGuid().ToString('N'))
$safeToDelete = $false
...
try {
    ...
    $sourceSkill = Join-Path $packageRoot '.apm/skills/cursor/SKILL.md'
    $installedSkill = Join-Path $resolvedScratch '.agents/skills/cursor/SKILL.md'
    if (-not (Test-Path -LiteralPath $installedSkill -PathType Leaf)) {
        throw 'APM installation did not deploy cursor/SKILL.md.'
    }
    ...
    Write-Output 'Cursor CLI package-root APM install smoke test: PASS'
}
finally {
