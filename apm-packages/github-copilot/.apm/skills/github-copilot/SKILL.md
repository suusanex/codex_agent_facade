---
name: github-copilot
description: Codex 自身は対象作業を実行しない。Skill より後のユーザー本文を Codex Agent Facade 経由で GitHub Copilot CLI へ委譲し、その結果を中継する。この Codex thread で GitHub Copilot に作業させ、Codex は薄い UI shell / relay として結果だけを返すときに使う。
user-invocable: true
...
この Skill が指定された turn で、Codex は planner / executor / reviewer / orchestrator ではない。Codex は GitHub Copilot に対する薄い UI shell / relay である。対象作業は GitHub Copilot が行い、Codex はその結果を中継する。
