...
$scratchPath = Join-Path $tempRoot ('devin-cli-apm-' + [guid]::NewGuid().ToString('N'))
$safeToDelete = $false
...
try {
    ...
    $sourceSkill = Join-Path $packageRoot '.apm/skills/devin-cli/SKILL.md'
    $installedSkill = Join-Path $resolvedScratch '.agents/skills/devin-cli/SKILL.md'
    if (-not (Test-Path -LiteralPath $installedSkill -PathType Leaf)) {
        throw 'APM installation did not deploy devin-cli/SKILL.md.'
    }
    ...
    Write-Output 'Devin CLI package-root APM install smoke test: PASS'
}
finally {
