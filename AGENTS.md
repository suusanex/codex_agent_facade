...
10. 要求にない汎用的なCLI機能を、ベストプラクティスや慣習だけを理由に追加しないこと。特に --check、--dry-run、--self-test、--force、診断用サブコマンド等をデフォルトでは追加しない。スクリプトが呼び出す外部I/Fに --dry-run がすでに有るなどの、明確に意味の有るケースでだけ追加を検討する。開発時の検証が必要なら、まず既存のテスト手段または外部テストを使用し、検証の都合だけで本番スクリプトの公開I/Fを増やさないこと。
